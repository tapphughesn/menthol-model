"""
Creates a bunch of simulation data for different initiation/cessation rates.

Later, that data is used to find the right initiation/cessation rate for calibrating to NHIS.
"""
...
def main(args):
    ...
    csvnsRR = pd.read_excel(os.path.join("..", "..", "smoking_mortality",
                            "current_smoker_mortality_vs_nonsmoker.xlsx")).to_numpy()[:, 1:]
    fsvcsRR = pd.read_excel(os.path.join("..", "..", "smoking_mortality",
                            "former_smoker_mortality_vs_current_smoker.xlsx")).to_numpy()[:, 1:]
    ...
    for reduction in np.linspace(0.04, 0.08, 41):
        ...
        if not os.path.isdir(os.path.join(save_dir, r_str)):
            os.mkdir(os.path.join(save_dir, r_str))
        if not os.path.isdir(os.path.join(save_dir, r_str, "numpy_arrays")):
            os.mkdir(os.path.join(save_dir, r_str, "numpy_arrays"))
        ...
        for _ in range(args.number_replications):
            ...
            s = Simulation(pop_df=pop_df,
                           ...
                           save_dir='../../nhis_calib_outputs/' + r_str + '/',
